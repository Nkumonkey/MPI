	...
	MPI_Request request;
		 ...
		 //当 前 行 是 自 己 进 程 的 任 务 — — 进 行 消 去
			 ...
			 for (int j = k + 1; j < N; j++)
				 A[k][j] = A[k][j] / A[k][k];
			 A[k][k] = 1.0;
			 MPI_Isend(A[k], N, MPI_FLOAT, next_proc, 2, MPI_COMM_WORLD,&request);
		}
		 //当 前 行 不 是 自 己 进 程 的 任 务 — — 接 收 消 息
			 else {
				 MPI_Irecv(A[k], N, MPI_FLOAT, pre_proc, 2, MPI_COMM_WORLD,&request);
				 //如 果 当 前 行 不 是 下 一 进 程 的 任 务， 需 将 消 息 进 行 传 递
